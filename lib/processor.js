...
const getAmountToPay = async (data) => {
    ...
    if (act.field === "amount") {
        ...
        const sum = newTransactions.reduce((acc, cur) => acc + cur.amount, 0) /100
        console.log("getting", act.value, "%", )
        amount = sum * act.value/100
    } else amount = 0;
...
async function executeAutomation(auto) {
    console.log("Running automation", auto.label)
    ...
    for (let act of auto.actions) {
        ...
        console.log("transfer amount: ghc " + amount + " from " + source + " to " + destination)
        ...
        try {
            ...
            // await sendText("Transaction finished for automation: " + auto.label)
            auto.lastRan = Date.now()
            await auto.save().then(() => console.log(`${auto.oid} last transaction hash set to ${auto.lastTransactionHash}`));

        } catch (err) {
            console.error(`${err.response.status} ${err.response.statusText}`);

            const finishTime = Date.now()
            let logs = err.response.data.message === "" ? "failed for an unknown reason": err.response.data.message
            logs = logs + " for automation: " + auto.label;

            const success= false
            auto.history = [...auto.history, {
                startTime, finishTime, success, logs
            }]
            auto.lastRan = Date.now()
            await auto.save().then(() => console.log(`${auto.oid} last transaction hash set to ${auto.lastTransactionHash}`));
        }
