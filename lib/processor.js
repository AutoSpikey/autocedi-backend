...
const getAmountToPay = async (data) => {
    ...
    if (act.field === "amount") {
        ...
        amount = newTransactions.reduce((acc, cur) => acc + cur.amount, 0) /100
    } else amount = 0;
...
async function executeAutomation(auto) {
    ...
    for (let act of auto.actions) {
        ...
        console.log("transfer amount: " + " from " + source + " to " + destination)
        ...
        try {
            ...
            const finishTime = Date.now()
            auto.history = [...auto.history, {
                startTime, finishTime, success: false, logs: err.response.data.message
            }]
            auto.lastRan = Date.now()
            await auto.save().then(() => console.log(`${auto.oid} last transaction hash set to ${auto.lastTransactionHash}`));
        }
