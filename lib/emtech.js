...
async function getAuthToken() {
    ...
    await redis.set(TOKEN_KEY, token, { "EX": "250" }) // expire token after 5 minutes
...
async function getTransactionsAfterHash(walletId, savedHash) {
    let page = 1;
    ...
    try {
        ...
        while (true) {
            ...
            for (const transaction of transactions) {
                // If savedHash is provided and found, break out of the loop
                if (savedHash && transaction.hash === savedHash) {
                    break;
                }
            ...
            if (savedHash && transactions.some(transaction => transaction.hash === savedHash)) {
                break;
            }
...
module.exports = {
    createWallet, transfer, getWallet, loadWallet, getTransactionsAfterHash, getTransactionHistory
}